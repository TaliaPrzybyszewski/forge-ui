...
const DEFAULTS: Record<string, Record<string, unknown>> = {
  button:     { color: '#7F77DD', variant: 'glow', size: 'md', radius: 8, hoverEffect: 'lift', shadow: 'glow', clickAnim: 'ripple', speed: 'normal', uppercase: false, fullWidth: false },
...
export function buildCLIFlags(componentId: string, props: Record<string, unknown>): CLIFlag[] {
...
export function buildCLIString(componentId: string, props: Record<string, unknown>): string {
  const flags = buildCLIFlags(componentId, props)
  const parts = [`npx prism-ui add ${componentId}`]
