...
const ICONS: Record<string,string> = {none:'',arrow:'→',arrowLeft:'←',star:'✦',plus:'+',check:'✓',bolt:'⚡',send:'↗',heart:'♡',moon:'◐',eye:'◎',lock:'⚿',flame:'✶',x:'✕',search:'⌕'}

const SIZE_PRESETS: Record<string,{paddingY:number,paddingX:number,textSize:number}> = {
...
export default function ButtonRenderer({ props: p }: { props: any }) {
  const col = p.color || '#7F77DD'
  const rgb = hexRgb(col)
  const btnRef = useRef<HTMLButtonElement>(null)

  const icon = ICONS[p.icon] || ''
  const txt = p.uppercase ? (p.text||'').toUpperCase() : (p.text||'Start a project')
  ...
  const V: Record<string,{bg:string,b:string,c:string}> = {
    ...
    ghost:   {bg:'transparent',      b:'rgba(255,255,255,.1)',c:'rgba(240,237,232,.5)'},
  ...
  const specWrap: React.CSSProperties = isSpectrum ? {
    position:'relative', borderRadius: p.radius+1,
    background:'conic-gradient(from var(--ang),#e24b4a,#EF9F27,#f5e642,#1D9E75,#378ADD,#7F77DD,#D4537E,#e24b4a)',
    padding:'1.5px',
    animation: p.disabled ? 'spin 3s linear infinite paused' : 'spin 3s linear infinite',
  ...
  const hoverMap: Record<string,string> = {lift:'translateY(-2px)',scale:'scale(1.04)',glow:'none',none:'none'}

  function handleMouseEnter(e: React.MouseEvent<HTMLButtonElement>) {
    const btn = e.currentTarget
    if (p.hoverEffect !== 'none') btn.style.boxShadow = shadowMap[p.shadow] ?? 'none'
    btn.style.transform = hoverMap[p.hoverEffect] ?? 'none'
    if (p.variant === 'glow') { btn.style.borderColor = `rgba(${rgb},.55)`; btn.style.color = lighten(col,80) }
    if (p.variant === 'ghost') { btn.style.background = 'rgba(255,255,255,.05)'; btn.style.color = '#f0ede8' }
  }
  function handleMouseLeave(e: React.MouseEvent<HTMLButtonElement>) {
    const btn = e.currentTarget
    btn.style.boxShadow = 'none'; btn.style.transform = 'none'
    btn.style.background = v.bg; btn.style.borderColor = v.b; btn.style.color = v.c
  }
  function handleClick(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    if (p.clickAnim === 'scale' && btnRef.current) {
      ...
      setTimeout(() => { if (btnRef.current) btnRef.current.style.transform = 'none' }, 120)
    }
  ...
  const ghostVariants = ['ghost','outline','soft'].filter(k => k !== p.variant).slice(0,2)
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:14}}>
      <div style={specWrap}>
        ...
        >
          {icon && p.iconPos === 'left' && <span style={{fontSize:textSize+2}}>{icon}</span>}
          <span>{txt}</span>
          {icon && p.iconPos === 'right' && <span style={{fontSize:textSize+2}}>{icon}</span>}
        </button>
      </div>
      <div style={{display:'flex',gap:8,opacity:.2}}>
        {ghostVariants.map(k => {
          const vv = V[k]
          return (
            <button key={k} style={{padding:'6px 14px',borderRadius:p.radius,fontSize:11,background:vv.bg,border:`1px solid ${vv.b}`,color:vv.c,fontFamily:'Inter,sans-serif',cursor:'pointer'}}>{k}</button>
          )
        })}
      </div>
    </div>
